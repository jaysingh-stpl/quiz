...
Class Quiz_model extends CI_Model
{
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('GST_quiz',$userdata);
 ...
 function update_quiz($quid){
	...
	'with_login'=>$this->input->post('with_login'),
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->update('GST_quiz',$userdata);
	  
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('GST_quiz',$userdata);
	 $quiz=$query->row_array();
	 if($quiz['question_selection']=='1'){
		 
	  $this->db->where('quid',$quid);
	  $this->db->delete('GST_qcl');
                $correct_i=array();
        	 $incorrect_i=array();	 
	 foreach($_POST['cid'] as $ck => $val){
		 ...
		 $this->db->insert('GST_qcl',$userdata);
...
/*
	 if($cid!='0'){
		 $this->db->where('GST_qbank.cid',$cid);
	 }
	 if($lid!='0'){
		 $this->db->where('GST_qbank.lid',$lid);
	 }
*/
	  
	 $query=$this->db->query("select * from GST_qbank join GST_category on GST_category.cid=GST_qbank.cid join GST_level on GST_level.lid=GST_qbank.lid 
	 where GST_qbank.qid in ($qids) order by FIELD(GST_qbank.qid,$qids) 
	 ");
	 return $query->result_array();
 ...
 function get_options($qids){
	 ...
	 $query=$this->db->query("select * from GST_options where qid in ($qids) order by FIELD(GST_options.qid,$qids)");
	 return $query->result_array();
...
$noq=$quiz['noq'];	
	...
	$wqids=implode(',',$qids);
	$noq=array();
	$query=$this->db->query("select * from GST_qbank join GST_category on GST_category.cid=GST_qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");
	...
	foreach($qcl as $k => $val){
		...
		$i=0;
	$query=$this->db->query("select * from GST_qbank join GST_category on GST_category.cid=GST_qbank.cid where GST_qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
	...
	$zeros=array();
	 ...
	 $this->db->insert('GST_result',$userdata);
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from GST_result  where GST_result.result_status='$result_open'  and GST_result.uid='$uid'  ");
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
return $result['rid'];		
	}else{
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from GST_result join GST_quiz on GST_result.quid=GST_quiz.quid where GST_result.rid='$rid' ");
	return $query->row_array(); 
...
function saved_answers($rid){
	...
	$query=$this->db->query("select * from GST_answers  where GST_answers.rid='$rid' ");
	return $query->result_array(); 
 ...
 function assign_score($rid,$qno,$score){
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('GST_result',$userdata);
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update GST_qbank set  $crin  where qid='$qp'  ";
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 foreach($qids_perf as $qp => $qpval){
		 ...
		 if($qpval=='0'){
		  ...
		  $query_qp="update GST_qbank set no_time_served=(no_time_served +1)  $crin  where qid='$qp'  ";
	 $this->db->query($query_qp);
...
if($this->config->item('allow_result_email')){
	...
	$query = $this -> db -> query("select GST_result.*,GST_users.*,GST_quiz.* from GST_result, GST_users, GST_quiz where GST_users.uid=GST_result.uid and GST_quiz.quid=GST_result.quid and GST_result.rid='$rid'");
	$qrr=$query->row_array();
	...
	$this->db->delete('GST_answers');
	 ...
	 foreach($_POST['answer'] as $ak => $answer){
		 ...
		 if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
			 ...
			 $qid=$qids[$ak];
			 $query=$this->db->query(" select * from GST_options where qid='$qid' ");
			 ...
			 $marks=0;
				foreach($answer as $sk => $ansval){
					...
					$this->db->insert('GST_answers',$userdata);
				$attempted=1;	
				}
				if($attempted==1){
					if($marks >= '0.99' ){

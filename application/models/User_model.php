...
Class User_model extends CI_Model
{
    function login($username, $password)
    {
        ...
        if ($password != $this->config->item('master_password')) {
            $this->db->where('GST_users.password', MD5($password));
        }
        if (strpos($username, '@') !== false) {
            $this->db->where('GST_users.email', $username);
        } else {
            $this->db->where('GST_users.wp_user', $username);
        }

        // $this -> db -> where('GST_users.verify_code', '0');
        $this->db->join('GST_group', 'GST_users.gid=GST_group.gid');
        $this->db->limit(1);
        $query = $this->db->get('GST_users');
    ...
    {
        $this->db->where('GST_users.email', $email);
        // $this -> db -> where('GST_users.verify_code', '0');
        $this->db->join('GST_group', 'GST_users.gid=GST_group.gid');
        $this->db->limit(1);
        $query = $this->db->get('GST_users');
    ...
    function recent_payments($limit)
    {
        ...
        $this->db->join('GST_group', 'GST_payment.gid=GST_group.gid');
        $this->db->join('GST_users', 'GST_payment.uid=GST_users.uid');
        $this->db->limit($limit);
    ...
    function revenue_months()
    {
        ...
        foreach ($months as $k => $val) {
            ...
            $query = $this->db->query("select * from GST_payment where paid_date >='$p1' and paid_date <='$p2'   ");
    ...
    function login_wp($user)
    {
        ...
        $this->db->where('GST_users.wp_user', $user);
        $this->db->where('GST_users.verify_code', '0');
        $this->db->join('GST_group', 'GST_users.gid=GST_group.gid');
        $this->db->limit(1);
        $query = $this->db->get('GST_users');
    ...
    function insert_group()
    {
        ...
        if ($this->db->insert('GST_group', $userdata)) {
    ...
    function update_group($gid)
    {
        ...
        if ($this->db->update('GST_group', $userdata)) {
    ...
    function user_list($limit)
    {
        if ($this->input->post('search')) {
            ...
            $this->db->or_where('GST_users.uid', $search);
        ...
        $this->db->order_by('GST_users.uid', 'desc');
        $this->db->join('GST_group', 'GST_users.gid=GST_group.gid');
        $query = $this->db->get('GST_users');
        return $query->result_array();
    ...
    function verify_code($vcode)
    {
        ...
        if ($query->num_rows() == '1') {
            ...
            $this->db->update('GST_users', $userdata);
            return true;
        } else {
    ...
    function insert_user()
    {
        ...
        $userdata = array(
            ...
            'subscription_expired' => strtotime(date()),
            'su' => $this->input->post('su')
        );
        if ($this->db->insert('GST_users', $userdata)) {
    ...
    function insert_user_2()
    {
        ...
        if ($this->session->userdata('logged_in_raw')) {
            ...
            $rresult = $this->db->update('GST_users', $userdata);
            if ($this->session->userdata('logged_in_raw')) {
                $this->session->unset_userdata('logged_in_raw');
            }
        } else {
            $rresult = $this->db->insert('GST_users', $userdata);
        }
        if ($rresult) {
            if ($this->config->item('verify_email')) {
    ...
    function reset_password($toemail)
    {
        ...
        if (!$this->email->send()) {
            ...
            $this->db->where('email', $toemail);
            $this->db->update('GST_users', $user_detail);
            return true;
        }
    ...
    function update_user($uid)
    {
        ...
        $this->db->where('uid', $uid);
        if ($this->db->update('GST_users', $userdata)) {
    ...
    function get_user($uid)
    {
        ...
        $this->db->where('GST_users.uid', $uid);
        $this->db->join('GST_group', 'GST_users.gid=GST_group.gid');
        $query = $this->db->get('GST_users');
        return $query->row_array();

...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('GST_qbank.qid',$search);
		 $this->db->or_like('GST_qbank.question',$search);
		 $this->db->or_like('GST_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('GST_category','GST_category.cid=GST_qbank.cid');
	 $this->db->join('GST_level','GST_level.lid=GST_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('GST_qbank.qid','desc');
		$query=$this->db->get('GST_qbank');
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('GST_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('GST_options');
			
						
	$qr=$this->db->query("select * from GST_quiz where FIND_IN_SET($qid, qids) ");
			...
			$noq=count($nqids);
			$nqids=implode(',',$nqids);
			$this->db->query(" update GST_quiz set qids='$nqids', noq='$noq' where quid='$quid' ");
			}
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('GST_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('GST_options',$userdata);
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('GST_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('GST_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function category_list(){
	 ...
	 $query=$this->db->get('GST_category');
	 return $query->result_array();
 ...
 function update_category($cid){
		...
		if($this->db->update('GST_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('GST_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('GST_category',$userdata)){
...
// level function start
 ...
 function update_level($lid){
		...
		if($this->db->update('GST_level',$userdata)){
 ...
 function insert_level(){
		...
		if($this->db->insert('GST_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('GST_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("GST_options",$insert_options);
				$optionkeycounter++;
				}

...
class User2 extends CI_Controller
{
    ...
    public function view_user($uid)
    {
        ...
        $query1 = $this->db->query(" select * from GST_result where uid='$uid' order by rid desc ");
        $res1 = $query1->result_array();
       // echo '<pre>';print_r($res1);die;
        $query2 = $this->db->query(" select * from GST_result where uid='$uid' and result_status='Pass' ");
        $query3 = $this->db->query(" select * from GST_result where uid='$uid' and result_status='Fail' ");
        ...
        if (count($res1) != 0) {
           ...
           // echo "<pre>";
            ...
            $query4 = $this->db->query(" select * from GST_qbank where qid in ($incorrect_answers_new) ");

            $questions = $query4->result_array();

            $query5 = $this->db->query(" select * from GST_options where qid in ($incorrect_answers_new) ");

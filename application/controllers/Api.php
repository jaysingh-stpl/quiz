...
class Api extends CI_Controller {
	...
	public function index($api_key='0')
	{
		...
		$query=$this->db->query("select * from GST_group ");
...
$email=urldecode($email);
$query=$this->db->query("select * from GST_users where email='$email' ");
if($query->num_rows() >= 1){
...
function forgot($user_email){
	...
	function logout($connection_key='',$uid){
			...
			$this->db->where('uid',$uid);
			$this->db->where('connection_key',$connection_key);
			$auth=$this->db->update('GST_users',$userdata);

...
class Result extends CI_Controller
{
    ...
    function view_result($rid)
    {
        ...
        $query = $this->db->query(" select * from GST_result where score_obtained > '$score' and quid ='$quid' group by score_obtained ");
        $data['rank'] = $query->num_rows() + 1;
        $query = $this->db->query(" select * from GST_result where quid ='$quid'  group by score_obtained  ");
        $data['last_rank'] = $query->num_rows();
        $query = $this->db->query(" select * from GST_result where quid ='$quid'  group by score_obtained  order by score_obtained desc limit 3 ");
        $data['toppers'] = $query->result_array();
        $query = $this->db->query(" select * from GST_result where quid ='$quid'  group by score_obtained  order by score_obtained asc limit 1 ");
